...
def calculateWidth(filamentNo,filamentData, params, plotIndividualProfiles, printResults):
	...
	while True: #n2 <  len(filamentData):	
		...
		print 'Processed chunk		= ', n2/12, 'of', len(filamentData)//avg_len
		print '######################################################################'
		...
		a		= np.amax(ydata)
		mu		= r[ np.argmax(ydata) ]
		pos_half	= np.argmin( np.abs( ydata-a/2 ) )
		...
		try:
			...
			for ii in range(len(r)): 
              			tot_2 += ydata[ii]*(r[ii] - np.mean(r))**2
                                tot_3 += ydata[ii]*(r[ii] - np.mean(r))**3
                                tot_4 += ydata[ii]*(r[ii] - np.mean(r))**4
               
                	var = math.sqrt(tot_2/np.sum(ydata))
        		mom3 = tot_3/np.sum(ydata)
        		mom4 = tot_4/np.sum(ydata)
			...
			co_eff,var_matrix	= curve_fit(gaus,r, ydata,p0=p01,absolute_sigma=True)	
			co_eff3,var_matrix3	= curve_fit(plum2,r, ydata,p0=p02)
			co_eff4,var_matrix4	= curve_fit(plum4,r, ydata,p0=p02)
			
			#Calculate Chi-squared 
		        noise			= noise_level  # 0.47/sqrt(3)/sqrt(12)
			num_freeParams		= 3  	        
			# gaussian fits
			chi_sq_gaus		= np.sum((ydata-gaus(r,*co_eff))**2) / noise**2
		        red_chi_sq_gaus		= chi_sq_gaus / (len(ydata) - num_freeParams)			
			
			# plummer 2 fits
			chi_sq_plum2		= np.sum((ydata-plum2(r,*co_eff3))**2) / noise**2	
			red_chi_sq_plum2	= chi_sq_plum2 / (len(ydata) - num_freeParams)	
			
			# plummer 4 fits
			chi_sq_plum4		= np.sum((ydata-plum4(r,*co_eff4))**2) / noise**2
			red_chi_sq_plum4	= chi_sq_plum4 / (len(ydata) - num_freeParams)
			...
			pl.savefig('/home/suri/development/filchap_1.0/c18o_north/plots/widthPlotFil' + str(filamentNo)+'_slice' + str(n2) +'.png', dpi=300)
			#pl.show()
